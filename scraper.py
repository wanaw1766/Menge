"""
scraper.py — AXIOM INTEL (Groq-only, robust weekly detection)
"""
...
class ChannelScraper:
    ...
    async def _image_looks_like_ff(self, image_data: bytes, image_mime: str) -> bool:
        try:
            prompt = (
                ...
                "Respond with JSON: {\"is_ff\": true/false}"
            )
            content = [{"type": "image_url", "image_url": {"url": f"data:{image_mime};base64,{base64.b64encode(image_data).decode()}"}},
            ...
            log.warning(f"FF check failed: {e}")
            return False
    ...
    async def _is_weekly_image_ai(self, image_data: bytes, image_mime: str) -> bool:
        ...
        try:
            ...
            log.warning(f"Weekly detection failed: {e}")
            return False
    ...
    async def _handle_ff_image(self, image_data: bytes, image_mime: str, caption: str,
        ...
        if is_weekly:
            ...
            if not result.get("approved"):
                ...
                log.info(f"Weekly rejected: {result.get('reason')}")
                return
            ...
            if not result.get("approved"):
                ...
                log.info(f"Daily rejected: {result.get('reason')}")
                return
